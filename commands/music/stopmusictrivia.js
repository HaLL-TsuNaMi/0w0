...
module.exports = class StopMusicTriviaCommand extends Command {
  ...
  run(message) {
    if (!message.guild.triviaData.isTriviaRunning)
      return message.say('No twivia is cuwwentwy wunning UmU');

    if (message.guild.me.voice.channel !== message.member.voice.channel) {
      return message.say("Join the trivia's channew and twy again UwU");
    }

    if (!message.guild.triviaData.triviaScore.has(message.author.username)) {
      return message.say(
        'You need to pawticipate in the twivia in owdew to end it UwU'
      );
    }
