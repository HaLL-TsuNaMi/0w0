...
function Application() {
  ...
  this.tune = settings => {
    ...
    for (let name in settings) {
      if (name === "strictRouting") {
        return utils.showError("tune() strictRouting is not supported anymore");
      }
  ...
  this.addRoute = (route, layers, callback) => {
    ...
    if (route.method) {
      if (!utils.validate.string(route.method)) {
    ...
    if (
      ...
      !utils.validate.object(route.match)
    ) {
    ...
    if (
      ...
      !utils.validate.object(route.query)
    ) {
  ...
  this.addRoutes = (routes, callbacks) => {
    for (let i = 0; i < routes.length; i++) {
      ...
      this.addRoute(routes[i], callbacks[obj][func]);
    }
  };
  ...
  this.processRequest = (req, res) => {
    ...
    self.router
      ...
      .then(route => {
        ...
        self.router.layers.applyLayers(route.id, req, res)
          .then(() => {
